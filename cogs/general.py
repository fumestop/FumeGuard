...
class General(commands.Cog):
    ...
    async def _uptime(self, ctx: discord.Interaction):
        """Shows how long has FumeGuard has been up for."""
        # noinspection PyUnresolvedReferences
        await ctx.response.defer(thinking=True)
    ...
    async def _web(self, ctx: discord.Interaction):
        """Shows the links to various FumeGuard resources on the web."""
        # noinspection PyUnresolvedReferences
        await ctx.response.defer(thinking=True)
    ...
    async def _invite(self, ctx: discord.Interaction):
        """Sends the link to invite FumeGuard to your server."""
        # noinspection PyUnresolvedReferences
        await ctx.response.defer(thinking=True)
    ...
    async def _vote(self, ctx: discord.Interaction):
        """Sends the link to vote for FumeGuard on Top.GG."""
        # noinspection PyUnresolvedReferences
        await ctx.response.defer(thinking=True)
    ...
    async def _review(self, ctx: discord.Interaction):
        """Sends the link to review FumeGuard on Top.GG."""
        # noinspection PyUnresolvedReferences
        await ctx.response.defer(thinking=True)
        ...
        view.add_item(
            discord.ui.Button(
                label="Review", url="https://fumes.top/fumeguard/review"
            )
        )
    ...
    async def _community(self, ctx: discord.Interaction):
        """Sends the link to the FumeGuard community server."""
        # noinspection PyUnresolvedReferences
        await ctx.response.defer(thinking=True)
